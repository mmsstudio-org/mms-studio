...
import { checkSlugUnique, updateBlog, deleteBlog } from '@/lib/firestore-service';
...
import {
  Calendar, User, Pencil, Save, X, Trash2, ArrowLeft, ShieldAlert,
  Loader2
} from 'lucide-react';
...
export default function BlogPostClient({ initialBlog }: BlogPostClientProps) {
  ...
  const [editTitle, setEditTitle] = useState(initialBlog.title);
  const [editSlug, setEditSlug] = useState(initialBlog.slug);
  const [editCoverUrl, setEditCoverUrl] = useState(initialBlog.coverImageUrl || '');
  const [editContent, setEditContent] = useState(initialBlog.content);
  const [editExcerpt, setEditExcerpt] = useState(initialBlog.excerpt || '');
  const [editAuthor, setEditAuthor] = useState(initialBlog.author);
  const [editStatus, setEditStatus] = useState<'draft' | 'published'>(initialBlog.status);
  const [editPublishedAt, setEditPublishedAt] = useState(msToDateTimeLocal(initialBlog.publishedAt));
  const [editTags, setEditTags] = useState(initialBlog.tags?.join(', ') || '');
  ...
  // Handle access for drafts
  const isDraft = blog.status === 'draft';
  const hasAccess = !isDraft || (!!user);
